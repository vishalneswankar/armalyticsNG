...
var ResizeService = class {
  ...
  subscribe(callback) {
    ...
    this._zone.runOutsideAngular(() => {
      this.subscription = (0, import_rxjs.fromEvent)(this._dom.getWindow(), "resize").pipe((0, import_operators.auditTime)(FRAME_DURATION)).subscribe(() => callback());
    });
  }
  unsubscribe() {
...
var ScrollableService = class {
  ...
  subscribe(callback) {
    ...
    this._zone.runOutsideAngular(() => {
      const observables = parents.map((p) => (0, import_rxjs.fromEvent)(p, "scroll").pipe((0, import_operators.auditTime)(FRAME_DURATION)));
      const subscriber = (e) => {
      ...
      this.subscription = (0, import_rxjs.merge)(...observables).subscribe(subscriber);
    });
  }
  unsubscribe() {
    if (!this.subscription) {
...
var PopupComponent = class {
  ...
  ngAfterViewChecked() {
    ...
    this._zone.runOutsideAngular(() => {
      ...
      this.repositionSubscription = (0, import_rxjs.from)(this.resolvedPromise).subscribe(this.reposition);
    });
  }
  ngOnDestroy() {
...
//# sourceMappingURL=chunk-JIDDKRYE.js.map
